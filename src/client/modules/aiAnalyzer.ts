 ...
 * Analyzes audio/video to detect highlight moments for clipping
 */

import type { AudioAnalysisData, AudioSample, VisualSample, HighlightSuggestion } from '@types/index';
...
export class AIAnalyzer {
  ...
  async analyzeVideoSimple(
    ...
    try {
      ...
      console.log(
        `Analyzing ${totalDuration}s video with ${Math.ceil(totalDuration / sampleInterval)} samples`
      );
      ...
      if (wasPlaying) void videoElement.play();
  ...
  private findVisualHighlights(
    ...
    for (let i = 0; i < numClips; i++) {
      ...
      const nearbySamples = samples.filter((s) => Math.abs(s.time - centerTime) < clipDuration);

      if (nearbySamples.length > 0) {
        const avgMotion =
          nearbySamples.reduce((a, b) => a + (b.motion || 0), 0) / nearbySamples.length;
  ...
  private generateFallbackSuggestions(
    ...
    for (let i = 0; i < numSuggestions; i++) {
      const startTime =
        (totalDuration / (numSuggestions + 1)) * (i + 1) - clipDuration / 2;
  ...
  /**
   * Extract audio data from video element (advanced method)
   * Note: May have CORS issues with video sources
   */
  private async extractAudioData(videoElement: HTMLVideoElement): Promise<AudioAnalysisData> {
  ...
  private findHighlights(
    ...
    const peaks: Array<{ time: number; score: number; energy: number }> = [];
    for (let i = 1; i < samples.length - 1; i++) {
      ...
      if (sample.energy > prev.energy && sample.energy > next.energy && sample.energy > threshold) {
        peaks.push({
          time: sample.time,
    ...
    for (const peak of peaks) {
      ...
      const overlaps = highlights.some(
        (h) => Math.abs(h.startTime - (peak.time - clipDuration / 2)) < minGap
      );
   ...
   */
  private getHighlightReason(peak: { score: number }): string {
