...
import type { VideoInfo, Caption } from '@types/index';
import { Utils } from './utils';

type SourceType = 'file' | 'youtube' | 'twitch' | null;

export class VideoLoader {
  private videoElement: HTMLVideoElement | null = null;
  private currentSource: File | string | null = null;
   ...
   */
  async loadFromFile(file: File): Promise<VideoInfo> {
   ...
   */
  async loadFromUrl(url: string): Promise<VideoInfo> {
    ...
    try {
      ...
      throw new Error(
        'Failed to download video. Make sure the download server is running (npm start).'
      );
    }
  }
   ...
   */
  private async downloadFromServer(url: string): Promise<VideoInfo> {
   ...
   */
  isLoaded(): boolean {
    return (this.videoElement?.readyState ?? 0) >= 2;
  }
   ...
   */
  private async extractClipBrowser(startTime: number, endTime: number): Promise<Blob> {
    return new Promise((resolve, reject) => {
  ...
  drawCaptions(
    ...
    const words = activeCaption.words || [
      { text: activeCaption.text, startTime: activeCaption.startTime, endTime: activeCaption.endTime },
    ];
    const y = height * 0.85;
